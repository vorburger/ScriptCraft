...
Blockly.JavaScript['command'] = function(block) {
	var fname = Blockly.JavaScript.valueToCode(block, 'param',
			Blockly.JavaScript.ORDER_ATOMIC);
	var statements_statements = Blockly.JavaScript.statementToCode(block,
			'statements');
	var code = "var lauro_extra = require('./lauro_extra');\ncommand( " + fname
...
Blockly.JavaScript['circle'] = function(block) {
	var value_radius = Blockly.JavaScript.valueToCode(block, 'radius',
			Blockly.JavaScript.ORDER_ATOMIC);
	var dropdown_material = block.getFieldValue('material');
	var dropdown_fill = block.getFieldValue('fill');
	// TODO: Assemble JavaScript into code variable.
	var code = "theDrone.cylinder" + dropdown_fill + "(" + dropdown_material
			+ "," + value_radius + ",1);\n";
	return code;
};

Blockly.JavaScript['rectangle'] = function(block) {
	var value_width = Blockly.JavaScript.valueToCode(block, 'width',
			Blockly.JavaScript.ORDER_ATOMIC);
	var value_lenght = Blockly.JavaScript.valueToCode(block, 'lenght',
			Blockly.JavaScript.ORDER_ATOMIC);
	var dropdown_material = block.getFieldValue('material');
	var dropdown_fill = block.getFieldValue('fill');
	var code = "theDrone.box" + dropdown_fill + "(" + dropdown_material + ","
			+ value_width + ",1," + value_lenght + ");\n";
	return code;
};
...
Blockly.JavaScript['mountain'] = function(block) {
	...
	var value_lenght = Blockly.JavaScript.valueToCode(block, 'lenght',
			Blockly.JavaScript.ORDER_ATOMIC);
	var value_height = Blockly.JavaScript.valueToCode(block, 'height',
			Blockly.JavaScript.ORDER_ATOMIC);
	var code = "lauro_extra.piramid(theDrone," + value_width + ","
			+ value_lenght + "," + value_height + ");\n";
	return code;
};

Blockly.JavaScript['spawner'] = function(block) {
	var dropdown_name = block.getFieldValue('NAME');
	var code = "Packages.net.canarymod.Canary.factory().entityFactory.newEntity(Packages.net.canarymod.api.entity.EntityType[('"
			+ dropdown_name
...
Blockly.JavaScript['alternator'] = function(block) {
	...
	var code = "lauro_extra.boxAlternate(theDrone," + value_width + ","
			+ value_lenght + "," + material1 + "," + material2 + ");\n";
	return code;
};
